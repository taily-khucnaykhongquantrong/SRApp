...
# Code below is roughly similar on Matlab code graciously provided by Anish Mittal,
# but is written from scratch (since most of the Matlab functions there
# don't have numpy equivalents).
# Training the model is not implemented yet,
# so we rely on pre-trained model parameters in file modelparameters.mat.

# Cite:
# Mittal, Anish, Rajiv Soundararajan, and Alan C. Bovik.
# "Making a completely blind image quality analyzer."
# Signal Processing Letters, IEEE 20.3 (2013): 209-212.
# """
...
# Cite:
# Dominguez-Molina, J. Armando, et al.
# "A practical procedure to estimate the shape parameter
# in the generalized Gaussian distribution.",
...
#     elif k < 0.671256:
#         return (1 / (2 * b3 * k)) * (
#             b1 - b2 * k - math.sqrt((b1 - b2 * k) ** 2 - 4 * b3 * (k ** 2))
#         )
#     elif k < 0.75:
#         # print "%f %f %f" % (k, ((3-4*k)/(4*c1)), c2**2 + 4*c3*log((3-4*k)/(4*c1)) )
#         return (1 / (2 * c3)) * (
#             c2 - math.sqrt(c2 ** 2 + 4 * c3 * math.log((3 - 4 * k) / (4 * c1)))
#         )
#     else:
#         print("warning: GGRF inverse of " + str(k) + " is not defined")
#         return numpy.nan
...
#         img_pair_products = img_norm * numpy.roll(
#             numpy.roll(img_norm, y_shift, axis=0), x_shift, axis=1
#         )
...
#     mu_sq = mu * mu
#     sigma = numpy.sqrt(
#         numpy.abs(gaussian_filter(img * img, sigma, mode="nearest") - mu_sq)
#     )
#     img_norm = (img - mu) / (sigma + 1)
#     return img_norm


# def niqe(imgPath: str):
#     img = imageio.imread(imgPath, as_type=).astype(numpy.float) / 255

#     model_mat = scipy.io.loadmat("utils/niqe/modelparameters.mat")
...
#         if scale != 1:
#             img_scaled = skimage.transform.rescale(img, 1 / scale, multichannel=False)
...
#     niqe_quality = math.sqrt(
#         (model_mu - features_mu).dot(
#             pseudoinv_of_avg_cov.dot((model_mu - features_mu).T)
#         )
#     )
